# Questions & Answers:
by: Ori Ben Nun | for: Shahaf @ Sett.ai
...
it'll be good enough as it's about the process".
...
through the "third of the time" rules I realized that I was totally off.
...
And this is the mindset I embraced for the second iteration.
...
(like Three.js Vs. Canvas2D, embedded sounds, JS/TS, etc.)
...
plan MD (both to save time and to better simulate automation),
...
to the others.
**The for me idea was: the best plan is the one that 
yields the best output for the next step in the process without intervention.**

### Q: How did you pick the initial implementation version you modified to create the final playable?

### A:

[this part describes only the process of the 2nd attempt]

Very similar idea to the one mentioned above, but this time I just needed to play it
(in Claude's playground in the chat itself) and test (in that order):
...
prompt adjustments didn't yield any better results, so I just stopped and decided to move on 
...
1. Many small changes in one go (via vibe coding or manual fixes directly in the HTML file)
2. Play
3. Iterate
...
(I also worked on the task from 3 different devices, 3 different OSs,
which makes time counting even harder lol).

I also know that the time I spent iterating manually over a single implementation is stretching it a bit,
but I hope that the process as a whole, including me realizing my mistakes and learning from them, 
can show the actual quality of the process.
